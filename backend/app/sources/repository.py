...
class SourceRepository:
    ...
    async def read_all(self, simulation_id: int) -> List[Source]:
        result = await self.session.execute(
            select(Source).where(Source.simulation_id == simulation_id)
        )
        return result.scalars().all()

    async def read(self, simulation_id: int, name: str) -> Source | None:
        result = await self.session.execute(
            select(Source).where(
    ...
    async def delete(self, simulation_id: int, name: str) -> Source | None:
        source = await self.read(simulation_id, name)
        if source:
